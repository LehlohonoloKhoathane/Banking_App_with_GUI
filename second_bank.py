...
class withdrawMoney:
    ...
    def submit(self):
        ...
        if cleaned_withdrawal_amount.replace(".", "", 1).isdigit() and float(cleaned_withdrawal_amount) > 0:
            ...
            current_balance = self.c.execute('SELECT balance FROM Bank WHERE account_name = ?', (account_name,)).fetchone()
            if current_balance:  # Check if a balance is fetched
                current_balance = current_balance[0]  # Access the balance value within the tuple
                print(f"Current Balance before withdrawal: {current_balance}")
                if withdrawal_amount > current_balance:
                    messagebox.showerror("Error", "Insufficient funds.")
                    return
                new_balance = round(current_balance - withdrawal_amount, 2)
                ...
                self.con.commit()
                transaction_data = (account_name, "Withdrawal", withdrawal_amount, datetime.now().strftime("%Y-%m-%d %H:%M"))
...
class depositMoney:
    ...
    def submit(self):
        ...
        if cleaned_deposit_amount.replace(".", "", 1).isdigit() and float(cleaned_deposit_amount) > 0:
            ...
            if current_balance:  # Check if a balance is fetched
                current_balance = current_balance[0]  # Access the balance value within the tuple
                new_balance = round(current_balance + deposit_amount, 2)
                self.c.execute('UPDATE Bank SET balance = ? WHERE account_name = ?', (new_balance, account_name))
                self.con.commit()
